...
function get_comment_authors_ratings() {
	global $wpdb, $post, $comment_authors_ratings, $comment_authors_ratings_results;
	if(!is_feed() && !is_admin()) {
...
function create_ratinglogs_table() {
	...
	add_option('postratings_template_vote', '%RATINGS_IMAGES_VOTE%' );
	add_option('postratings_template_text', '%RATINGS_IMAGES%' );
	add_option('postratings_template_none', '%RATINGS_IMAGES_VOTE%' );
	...
	add_option('postratings_template_permission', '%RATINGS_IMAGES%' );
